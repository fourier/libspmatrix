    ...
    else
    {
      ...
      else
      {
        ...
        for (i = 0; i < 3; ++ i)
        {
          ...
          /* memcpy(x,b,mtx.rows_count*sizeof(double)); */
          portable_gettime(&t1);
          sp_matrix_yale_solve_cgs(&mtx,b,b,&iter,&tolerance,x);
          portable_gettime(&t2);
          printf("Solving SLAE using Conjugate Gradient Squared method");
          printf(" with tolerance %e(iterations: %d) time: ",
                 tolerance,iter);
          print_time_difference(&t1,&t2);
          printf("SLAE using Conjugate Gradient Squared with tolerance");
          printf(" %e(iterations: %d) max error: ",desired_tolerance[i],iter);
          print_error(x0,x,mtx.rows_count);
        }
        for (i = 0; i < 3; ++ i)
